        ...
        <div class="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            <div>
                ...
                <h2 class="font-semibold text-2xl text-slate-900 leading-tight">
                    {{ __('Assignment Role Detail') }}
                </h2>
                <p class="mt-1 text-sm text-slate-600">Role: {{ $role }}</p>
            </div>
        ...
        <div class="w-full">
            <div class="assignment-detail-card overflow-hidden">
                <div class="assignment-detail-band border-b border-gray-200 px-5 py-4">
                    <h3 class="text-lg font-semibold text-gray-900">Users in this Role</h3>
                    <p class="mt-1 text-sm text-gray-500">Assigned required modules and urgency for each learner.</p>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 text-sm">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-5 py-3 text-left font-medium text-gray-500">Assigned Required</th>
                                ...
                                <th class="px-5 py-3 text-left font-medium text-gray-500">Modules</th>
                            </tr>
                        </thead>
                                ...
                                <tr>
                                    <td class="px-5 py-3">
                                        <div class="font-medium text-gray-900">
                                            <a href="{{ route('app.admin.assignments.user', ['user' => $row['id']]) }}" class="text-indigo-600 hover:text-indigo-500">
                                                {{ $row['name'] }}
                                            </a>
                                        </div>
                                        <div class="text-gray-500">{{ $row['email'] }}</div>
                                    </td>
                                    <td class="px-5 py-3 text-gray-600">{{ $row['assigned_required_count'] }}</td>
                                    ...
                                    <td class="px-5 py-3 text-gray-600">
                                        @if ($row['modules']->isEmpty())
                                            None
                                        @else
                                            {{ $row['modules']->map(fn ($module) => $module['title'].' ('.$module['urgency'].')')->join(', ') }}
                                        @endif
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="6" class="px-5 py-4 text-gray-500">No users found for this role.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
